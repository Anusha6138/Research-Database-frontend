...
import  './conferenceForm.css' // Assuming you have a similar CSS file for styling
import { useAuth } from  "../context/AuthContext"
import { Navigate } from "react-router-dom";
export default function ConferenceForm({ConferenceIn}) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        const response = await axios.post('http://127.0.0.1:5000/api/conference', formData, {
          headers: {
  ...
  return (
    <Container className="conference-form-container d-flex justify-content-center align-items-center" style={{ minHeight: '100vh' }}>
      <Card className="p-4 conference-form-card">
        <h2 className="text-center mb-4">Conference Submission Form</h2>
